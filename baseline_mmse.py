...
def compute_mmse_beamformer(mat_H, K=4, N=4, P=10, noise_power=1, device=th.device("cuda:0" if th.cuda.is_available() else "cpu")):
...
def compute_mmse_beamformer_relay(mat_HTFG, mat_H, mat_F, K=4, N=4, P=10, noise_power=1, device=th.device("cuda:0" if th.cuda.is_available() else "cpu")):

...
def test_mmse_beamforming(N,K):
    env_mimo = MIMOEnv(K=K, N=N, num_env=4096)
    test_P = [10 ** 0.5, 10 ** 1, 10 ** 1.5, 10 ** 2, 10 ** 2.5]
    
    ## Test MMSE beamformer
    for P in test_P:
        env_mimo.reset(test=True, test_P=P)
        W, sr = compute_mmse_beamformer(env_mimo.mat_H, K=K, N=N)
        print('SNR =', P**2)
        print('sumrate =', sr.mean().item())
